...
def make_single_scene(output):
    ...
    # minimum kernel size adjustment
    output["gaussian"][0].mininum_kernel_size *= output["scale"][0,0].item()
    adjusted_scale = torch.maximum(
        adjusted_scale,
        torch.tensor(
    ...
    # return JUST the prepared gaussian, no merge
    return output["gaussian"][0]

def make_scene_no_merge(*outputs, in_place=False):
    if not in_place:
        outputs = [deepcopy(output) for output in outputs]
    ...
    for output in outputs:
        ...
        # 1. move gaussians to scene frame of reference
        PC = SceneVisualizer.object_pointcloud(
            points_local=output["gaussian"][0].get_xyz.unsqueeze(0),
            quat_l2c=output["rotation"],
            trans_l2c=output["translation"],
        ...
        # 2. rotate based on object rotation
        output["gaussian"][0].from_rotation(
            quaternion_multiply(
                quaternion_invert(output["rotation"]),
                output["gaussian"][0].get_rotation,
        ...
        # 3. scale adjustment and minimum kernel size handling
        scale = output["gaussian"][0].get_scaling
        adjusted_scale = scale * output["scale"]
        ...
        # 4. append the fully positioned, transformed gaussian
        scenes.append(output["gaussian"][0])
...
def ply_to_png(
    ...
    """
    Convert a PLY point cloud to a 2D scatter PNG.
    ...
    plt.grid(True, linestyle='--', alpha=0.5)  # optional: adds a grid for scale
    plt.tight_layout()
